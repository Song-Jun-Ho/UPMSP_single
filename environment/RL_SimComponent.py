...
###################################### 강화학습 모델과의 비교를 위한 SPT ####################################################

class Part(object):
    def __init__(self, name, type, process_time, weight, K):
        # 해당 Part 번호
        self.id = name
        ...
        self.process_time = np.average(process_time)
        ...
        self.p_ij = process_time
        self.max_process_time = np.max(np.array(self.p_ij))
        self.min_process_time = np.min(np.array(self.p_ij))
    ...
    def set_due_date(self, arrival_time):
        self.due_date = arrival_time + self.K * self.process_time


class Source(object):
    def __init__(self, env, IAT, weight, job_type, process_time, model, process_list, machine_num, part_num, K):
        self.env = env
        self.name = 'Source'
    ...
    def run(self):
        while True:
            ...
            part = Part(name='job{0}_{1}'.format(jb_type, self.generated_job_types[jb_type]), type=jb_type,
                        process_time=p, weight=w, K=self.K)
            ...
            inter_arrival_time = np.random.uniform(1, self.IAT)
...
class Process(object):
    ...
    def _to_machine(self):
        ...
        while True:
            ...
            yield self.parts_exist      # parts가 buffer에 존재할 시 열리는 valve
...
class Machine(object):
    ...
    def work(self, part, machine_store):
        ...
        proc_time = np.random.triangular(left=proc_time*0.9, mode=proc_time, right=proc_time*1.1)
